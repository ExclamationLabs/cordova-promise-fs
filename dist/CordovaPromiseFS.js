...
/***/ function(module, exports) {
	...
	function __createDir(rootDirEntry, folders, success,error) {
	  ...
	  module.exports = function(options){
		...
		// FCB: Filetransfer now has to be polyfilled for Cordova since as of cordova-ios 6, it has been removed. It was deprecated back in version 3.3
		  if (isCordova){
			...
			return fs.then(function(fs){
				if(!folders) {
				  ...
				  folders = folders.split('/').filter(function(folder) {
					return folder && folder.length > 0 && folder !== '.' && folder !== '..' && !folder.includes(':');
				  });
		...
		function file(path,options){
		  return new Promise(function(resolve,reject){
			...
			options = options || {};
			return fs.then(function(fs){
			  //Make sure the root path is removed
			  path = path.replace(CDV_URL_ROOT,'')
		...
		if(options.debug) console.log('toURLSync(localPath): ' + toURLSync(localPath))
